...
@wang_framework_2019 proposed one of the first systematic ML frameworks for fare prediction.
...
@aguirregabiria_dynamic_2010 model the US airline market as a dynamic entry-deterrence game using a computationally tractable structural estimation approach.
...
@ciliberto_collusive_2019 test for collusive pricing using DB1B data from 1993-2016, exploiting the multimarket contact structure of the US airline industry.
...
@ciliberto_collusive_2019 find that a 10% increase in multimarket contact between two carriers leads to a 3.5-5.3% decrease in pairwise fare differences within a market.
This is a clear empirical signature of coordinated pricing behavior.
They also find that code-share agreements (where two airlines sell tickets on each other's flights) reduce fare variability by approximately 1.5%.
...
@kerkemezos_price_2023 document an important path-dependence phenomenon in airline pricing.
...
A model trained on aggregated data appears to perform well (as in @wang_framework_2019 with R² = 0.87) but is actually solving an easier problem.
...
Consistent with the theoretical and empirical predictions of @ciliberto_collusive_2019 and @kerkemezos_price_2023, we construct several route-level competition measures at the route-quarter level.
...
@kerkemezos_price_2023 document that LCC entry causes legacy incumbents to price below competitive market averages, making LCC presence a strong predictor of fare suppression independent of the HHI level.
...
These competition effects are consistent in direction and magnitude with the structural estimates in @ciliberto_collusive_2019.
...
This is consistent with the oligopolistic market structure documented by @aguirregabiria_dynamic_2010, where brand loyalty, network benefits, and strategic entry deterrence allow legacy carriers to sustain large fare premia.
...
This finding validates the T-100 data integration strategy and confirms cost passthrough dynamics described in @saxon_airline_nodate.
...
All three effects are directionally consistent with the predictions of @ciliberto_collusive_2019 and @kerkemezos_price_2023.
The path-dependence findings of @kerkemezos_price_2023 suggest that `carrier_route_share` captures not just current market power but accumulated competitive history.
...
A two-level graph incorporating both airport nodes and city-pair nodes could capture the metropolitan-area substitutability dynamics documented by @aguirregabiria_dynamic_2010, where ORD and MDW serve as imperfect substitutes for Chicago-area travelers and competitive pressure from one propagates to the other.
